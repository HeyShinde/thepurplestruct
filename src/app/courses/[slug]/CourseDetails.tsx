...
export function CourseDetails({ course }: { course: CourseDetailsType }) {
      ...
      <div className="relative w-full overflow-hidden py-20 px-4 flex flex-col md:flex-row md:items-center md:justify-between gap-8">
        <div className="flex-1 flex flex-col justify-center items-start md:items-start z-10 max-w-2xl">
          <div className="flex gap-2 mb-3">
            {badges.map((badge, i) => (
              <Badge key={i} variant="outline" className="font-body bg-purple-700/20 text-purple-200 border-purple-400/40 font-semibold uppercase tracking-wide">{badge}</Badge>
            ))}
          </div>
          <h1 className="font-heading text-4xl md:text-5xl font-bold mb-2 text-white leading-tight">{course.title}</h1>
          <p className="font-body text-lg text-neutral-300 mb-4">{course.description}</p>
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="font-body text-base font-semibold text-white">{tutor.name}</span>
            <span className="font-body text-purple-300 text-sm">Instructor</span>
          </div>
        </div>
      ...
      <div className="container mx-auto py-10 px-4 grid grid-cols-1 md:grid-cols-3 gap-8 flex-1 w-full">
        ...
        <div className="md:col-span-2 order-last md:order-first">
          <div className="space-y-8">
            ...
            {/* Requirements */}
            <div className="bg-white/5 rounded-2xl p-6 border border-purple-400/20 shadow-lg">
              <h2 className="font-heading text-xl font-semibold text-purple-400 mb-4">Requirements</h2>
              <ul className="list-disc pl-6 text-neutral-300">
                {requirements.map((req, i) => (
                  <li key={i} className="font-body">{req}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-purple-400/20 shadow-lg">
              <h2 className="font-heading text-2xl font-semibold text-purple-400 mb-4">Course Content</h2>
              <div className="space-y-4">
                {sortedSections.length > 0 ? (
                      ...
                      <Collapsible.Root key={section._id} open={isOpen} onOpenChange={() => setOpenSection(isOpen ? null : section._id)}>
                        <div className="bg-black/40 rounded-lg border border-purple-400/20">
                          <Collapsible.Trigger asChild>
                            ...
                            >
                              <span className="font-heading text-xl font-semibold text-purple-400 group-hover:text-purple-300 transition-colors">{section.title}</span>
                            ...
                            <ul className="mt-2 space-y-2">
                              {sortedLessons.length > 0 ? (
                                sortedLessons.map((lesson) => (
                                  <li key={`${section._id}-${lesson._id}`} className="font-body text-neutral-300 pl-2 border-l-2 border-purple-400/20 hover:border-purple-400/60 transition-colors flex items-center gap-2">
                                ...
                                <li className="font-body text-neutral-300">No lessons found</li>
                              )}
                            </ul>
                          </Collapsible.Content>
                        </div>
                  ...
                  <div className="font-body text-neutral-300">No sections found</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white/5 rounded-2xl p-6 border border-purple-400/20 shadow-lg flex flex-col md:flex-row gap-6 items-center">
              ...
              <div>
                <h3 className="font-heading font-bold text-lg text-purple-400">{tutor.name}</h3>
                <p className="font-body text-sm text-neutral-300 mb-2">{tutor.bio}</p>
        ...
        <div className="md:col-span-1 order-first md:order-last">
          <div className="sticky top-4 bg-black/80 backdrop-blur-sm rounded-2xl p-6 border border-purple-400/20 shadow-lg flex flex-col gap-6">
            <p className="font-heading text-3xl font-bold text-purple-400 mb-2">{course.price === 0 ? 'Free' : `$${course.price}`}</p>
            {isEnrolled ? (
              <Link
                href={`/courses/learn/${course._id}`}
                className="font-heading block w-full bg-purple-600 text-white text-center px-6 py-3 rounded-lg font-semibold hover:bg-purple-700 transition"
              >
                Go to Course
              </Link>
            ) : (
              <button
                ...
                disabled={enrolling}
                className="font-heading block w-full bg-purple-600 text-white text-center px-6 py-3 rounded-lg font-semibold hover:bg-purple-700 transition disabled:opacity-50"
              >
                {enrolling ? "Enrolling..." : "Enroll Now"}
              </button>
            )}
            <ul className="text-gray-300 text-sm space-y-2 mb-2">
              <li className="font-body flex items-center gap-2"><FaClock className="text-purple-400" /> Lifetime access</li>
              <li className="font-body flex items-center gap-2"><FaGlobe className="text-purple-400" /> English</li>
            </ul>
            {/* Share Buttons */}
